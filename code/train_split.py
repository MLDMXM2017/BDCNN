...
def train_fold(fold,record_path,size,is_split,is_augment):
    ...
    if is_split==True:
        trainset= GetData_split('../flow0/'+fold+'/train','trn_label.txt',transform_train,4)
        valset= GetData_split('../flow0/'+fold+'/train','val_label.txt',transform_train,4)
        testset = GetData_split('../flow0/'+fold+'/test','label.txt',transform_test,4)
    else:
        dataset= GetData_raw('../flow0/'+fold+'/train',transform_train,4)
        testset = GetData_raw('../flow0/'+fold+'/test',transform_test,4)
    ...
    isda_criterion = ISDALoss(int(model.feature_num), class_num,[0.15,0.425,0.425]).cuda()
...
def train(train_loader, model, fc, criterion, optimizer, epoch,record_file):
    ...
    #the dataloader of four inputs u,v,os,vis
    for i, (x,x2,x3,x4,target) in enumerate(train_loader):
        target = target.cuda()
        x=x.cuda()
        x2 = x2.cuda()
        x3 =x3.cuda()
...
def validate(val_loader,model, fc, criterion, epoch,record_file):
    ...
    for i, (x,x2,x3,x4,target) in enumerate(val_loader):
        target = target.cuda()
        x=x.cuda()
        x2 = x2.cuda()
        x3=x3.cuda()
    ...
    for fold in folds:
        ...
        if fold!='total' and not os.path.exists(record_path):
        	print(fold+' starts training!')
                train_fold(fold,record_path,args.size,args.is_split,args.is_augment)
